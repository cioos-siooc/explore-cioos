...
def harvester(urls:str = typer.Option(...,help="harvest from these erddap servers, comma separated"),
              ...
              dataset_ids:str = typer.Option("",help="only harvest these dataset IDs. Comma separated list"), 
    ...
    try:
        ...
        if (not y):
            if(typer.confirm("Would you like to load harvested data into the CDE database?")):
...
def db_loader(folder:str = typer.Option("harvest",help="folder with the CSV output files from harvesting"),
              verbose: bool = typer.Option(False, help="Enable verbose output."),
              y: bool = typer.Option(False, "--yes", "-y", help="proceed with out prompts")
              ):
    ...
    try:
        main(folder)
        if (not y):
            if(typer.confirm(f"Would you like to flush all Redis Cache on {envs['REDIS_HOST']}:{envs['REDIS_PORT']} ?")):
