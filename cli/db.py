...
def init(verbose: bool = typer.Option(False, help="Enable verbose output.")):
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            print(
                "🔄 Creating set_constraints() and drop_constraints() funtions",
                end="\r",
            )
...
def removeData(
    ...
    y: bool = typer.Option(
        ...
        "--yes",
        "-y",
        help="Skip confirmation prompts and proceed with data but keeps database structure.",
    ),
):
    if not y:
...
@app.command(
    help="Clean the CDE database by dropping all structures. Optionally reinitialize the database."
)
def clean(
    verbose: bool = typer.Option(False, help="Enable verbose output."),
    y: bool = typer.Option(
        ...
        "--yes",
        "-y",
        help="Skip confirmation prompts and proceed with cleaning and reinitialization.",
    ),
):
    if not y:
        if typer.confirm(
            "This will clear all CDE data including all structures. Do you want to continue?"
        ):
            drop_db(verbose)
