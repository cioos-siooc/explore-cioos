...
@app.command()
def flushall(
            host:str = typer.Option(envs['REDIS_HOST'], help="Default loaded from env REDIS_HOST"),
            port:int = typer.Option(envs['REDIS_PORT'], help="Default loaded from env REDIS_PORT")):
...
def flushdb( 
            host:str = typer.Option(envs['REDIS_HOST'], help="Default loaded from env REDIS_HOST"),
            port:int = typer.Option(envs['REDIS_PORT'], help="Default loaded from env REDIS_PORT"),
            db: int = typer.Option(0, help="Database number eg(0-15), Default 0",
            )):
