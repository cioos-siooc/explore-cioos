...
select distinct unnest(organizations) from cioos_api.datasets ON CONFLICT DO NOTHING;

-- convert organization list of names into list of pks
with orgs as(
select d.pk,(
select array_remove(array_agg((select case when name=any(organizations) then pk end)),null) from cioos_api.organizations) as asdf from cioos_api.datasets d)
