...
dataset_is_realtime( coverage_time_max, last_updated_at )
...
(from the allDatasets listing, or the time dimension for grids);
last_updated_at is when the harvest ran. A dataset whose newest data is less
than a day behind its own harvest was live at that moment.
...
filter.
  ...
  SELECT dataset_is_realtime(coverage_time_max, last_updated_at)
    FROM cde.datasets;
...
$$
  SELECT coverage_time_max IS NOT NULL
     ...
     AND coverage_time_max >= last_updated_at - interval '1 day';
$$;
