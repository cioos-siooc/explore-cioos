...
| `3_ckan_records_migration.sql` | `CREATE TABLE IF NOT EXISTS cde.ckan_records` — the CKAN catalogue snapshot behind the harvest dashboard's coverage report. An idempotent table migration (not a function file) so live databases pick the table up without the volume drop this README otherwise requires |
...
| `7_constraints.sql` | `drop_constraints()` / `set_constraints()` |
| `8_range_functions.sql` | `range_intersection_length()` (download estimates), `day_union_days()` / `day_range_overlap_days()` (the day-set metric) |
