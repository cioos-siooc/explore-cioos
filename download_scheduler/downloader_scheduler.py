...
def get_a_download_job():
    ...
    rs = engine.execute("SELECT * FROM cioos_api.download_jobs WHERE status='open' ORDER BY time ASC LIMIT 1")
    return rs.fetchone()
...
def run_download(row):
    ...
    try:
        ...
        result = downloader_wrapper.parallel_downloader(
            json_blob=downloader_input, output_folder=output_folder, create_pdf=create_pdf
        )

        # Download Completed Update Status
        update_download_jobs(pk,{"status":"completed","downloader_output":str(result)})
       
    except Exception as e:
        # update_download_jobs(pk,{"status":"failed", "downloader_output":str(e)})
        update_download_jobs(pk,{"status":"open", "downloader_output":str(e)})
        print(e)

def update_download_jobs(pk,row):
    for key,value in row.items():
        engine.execute(f"UPDATE cioos_api.download_jobs SET {key}='{value}' WHERE PK={pk}")
