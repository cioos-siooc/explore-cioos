...
// Second segment of the top bar's Datasets/Filters pill: a quick way into the
...
// separate state of its own. Icon-only like the other middle segments in that
// pill; the popover it opens is a fresh mount each time (DropdownButton only
  ...
  return (
    <DropdownButton
      ...
      toggleClassName={classNames(
        "topBarButton topBarIconButton topBarSearchToggle",
        {
          active: menuOpen,
      ...
      // The button sits left-of-centre in the pill (Datasets | Search | Time
      // distribution | Bounding box | Filters), but the popover is nearly as
      // wide as the brand card above it — centering on the button itself
      // would leave it visibly off-centre under that card. The whole top bar
      // is centred on the viewport's own midline (see TopControls/styles.css),
      // so anchoring there instead lines the popover up with the card
      // regardless of where in the pill the button sits.
