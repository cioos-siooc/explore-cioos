...
const VIKING = {
  ...
  columnMeta: [
    ...
    },
    {
      ...
      standard_name: "obs_lat",
    },
    {
      name: "obs_lon",
      type: "float",
      long_name: "Observation longitude",
      standard_name: "obs_lon",
    },
    {
      name: "latitude",
      type: "float",
    ...
    },
    {
      name: "TE90_01",
      type: "float",
      long_name: "Temperature (1990 scale)",
...
test("the six Viking measurements are the panel candidates", () => {
  ...
  assert.deepEqual(
    measurements.map((v) => v.columnName),
...
test("without columnMeta the same six measurements are still found", () => {
  const measurements = measurementsOf(
    variablesFrom(VIKING_NO_META, VIKING_DATASET),
  );
  assert.deepEqual(
    measurements.map((v) => v.columnName),
    ["TE90_01", "CNDC_01", "PRES_01", "PSAL_01", "FLOR_01", "DOXY_01"],
...
test("a QC column is dropped via ancillary_variables, whatever it is called", () => {
  ...
  assert.deepEqual(
    measurementsOf(variablesFrom(table, {})).map((v) => v.columnName),
    ["chlorophyll"],
  );
});
...
test("depth is downward by declaration, and by CF convention without it", () => {
  ...
  assert.equal(
    isDownwardVertical({ columnName: "altitude", positive: "up" }),
    false,
  );
...
test("the harvest wins where the two disagree", () => {
  ...
  const index = byColumnName(
    variablesFrom(VIKING, {
      ...VIKING_DATASET,
      trajectory_id_variable: "station_id",
    }),
  );
  assert.equal(index.get("station_id").cfRole, "timeseries_id");
});
