...
describe("ScientificNameFilter", () => {
  ...
  it("shows the no-results message once an empty search settles", async () => {
    renderWithProviders(
      <ScientificNameFilter scientificNamesSelected={[]} setScientificNamesSelected={() => {}} searchTerms="" />,
    );
    await waitFor(() =>
      expect(
  ...
  it("renders suggestions from /scientificNames once the debounce settles", async () => {
    ...
    renderWithProviders(
      <ScientificNameFilter scientificNamesSelected={[]} setScientificNamesSelected={() => {}} searchTerms="orca" />,
    );
    await waitFor(() => expect(screen.getByText("Orcinus orca")).toBeInTheDocument(), {
      timeout: 2000,
    });
    ...
    expect(document.querySelector(".scientificNameOptionDetail")).toHaveTextContent(
      "Killer whale",
    );
  });
  ...
  it("filters out ranks too broad to search reliably (e.g. Order, Class)", async () => {
    ...
    renderWithProviders(
      <ScientificNameFilter scientificNamesSelected={[]} setScientificNamesSelected={() => {}} searchTerms="x" />,
    );
    await waitFor(() => expect(screen.getByText("Orcinus orca")).toBeInTheDocument(), {
      timeout: 2000,
    });
  ...
  it("clicking an unselected suggestion adds it to the selection", async () => {
    ...
    const option = await screen.findByText("Orcinus orca", {}, { timeout: 2000 });
  ...
  it("pins already-selected names at the top, ahead of new suggestions", async () => {
    ...
    const names = [...document.querySelectorAll(".scientificNameOptionName")].map(
      (el) => el.textContent,
    );
    expect(names.indexOf("Orcinus orca")).toBeLessThan(names.indexOf("Gadus morhua"));
  });
});
