...
export default function HarvestDownloadJob() {
  ...
  if (error || !data) {
    return (
      <HarvestLayout breadcrumbs={breadcrumbs}>
        <div className="harvest-muted">
          {t("harvest.downloads.jobNotFound")}
        </div>
      </HarvestLayout>
    );
  }
  ...
  return (
    <HarvestLayout breadcrumbs={breadcrumbs}>
      ...
      >
        <tbody>
          <tr>
            ...
            <td>
              <span
                className={`harvest-status harvest-status-${jobBadgeClass(job.status)}`}
              >
                {job.status}
              </span>
            </td>
            ...
            <td>
              ...
              {job.estimate_size != null && (
                <span className="harvest-muted harvest-text-sm">
                  {" "}
                  (
                  {t("harvest.downloads.estimated", {
                    size: fmtBytes(job.estimate_size),
                  })}
                  )
                </span>
              )}
      ...
      {job.error_message && (
        <>
          <h2 className="harvest-section-title">
            {t("harvest.downloads.jobError")}
          </h2>
          <pre className="harvest-mono harvest-error-block">
            {job.error_message}
          </pre>
        </>
      )}
      ...
      {datasets.length === 0 ? (
        <div className="harvest-muted">
          {t("harvest.downloads.noDatasetDetail")}
        </div>
      ) : (
            ...
            {datasets.map((d) => {
              ...
              return (
                <tr key={`${d.erddap_url}|${d.dataset_id}`}>
                  ...
                  <td>
                    <span
                      className={`harvest-status harvest-status-${datasetBadgeClass(d.status)}`}
                    >
                      {d.status}
                    </span>
                  </td>
                  <td className="harvest-num">{fmtBytes(d.file_size)}</td>
                  <td
                    className={`harvest-muted harvest-text-sm ${reason ? "harvest-error-text" : ""}`}
                    title={reason || ""}
                  >
                    {reason
                      ? `${reason.slice(0, 140)}${reason.length > 140 ? "…" : ""}`
                      : "—"}
                  </td>
                </tr>
              );
