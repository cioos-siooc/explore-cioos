    ...
    <div className="harvest-card-counts" style={{ marginBottom: "1rem" }}>
      ...
      <span className="harvest-muted harvest-text-sm harvest-self-center">
        {t("harvest.downloads.lastRequest", {
          date: fmtDt(summary.last_request_at),
        })}
      </span>
    </div>
  );
  ...
  return (
    <tr>
      <td>
        ...
        {reason && (
          <div
            className="harvest-muted harvest-text-xs harvest-error-text"
            style={{ marginTop: "2px" }}
            title={reason}
          >
            {reason.slice(0, 110)}
      ...
      <td
        className="harvest-muted harvest-text-sm"
      ...
      <td>
        <span
          className={`harvest-status harvest-status-${datasetBadgeClass(row.last_status)}`}
        >
          {row.last_status}
        </span>
      </td>
      <td className="harvest-num">{row.n_attempts}</td>
      <td className="harvest-num">{row.n_ok}</td>
      <td
        className={`harvest-num ${Number(row.n_failed) > 0 ? "harvest-error-text" : ""}`}
      >
        {row.n_failed}
      </td>
      <td className="harvest-muted harvest-num">{row.n_empty}</td>
      <td className="harvest-muted harvest-num">{row.n_ignored}</td>
      <td className="harvest-num">{fmtBytes(row.total_bytes)}</td>
      <td className="harvest-muted harvest-text-sm">
        {fmtDt(row.last_attempt_at)}
      </td>
    </tr>
  );
}
  ...
  return (
    <HarvestLayout breadcrumbs={breadcrumbs}>
      ...
      <h2 className="harvest-section-title">
        {t("harvest.downloads.datasetsTitle")}
      </h2>
      ...
      <div className="harvest-filter-bar">
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") applyFilters(statusFilter, q);
          }}
        />
        <button
          className="harvest-text-md harvest-clickable harvest-btn-sm"
      ...
      {loadingDatasets ? (
        ...
        <table className="harvest-table">
          <thead>
            <tr>
              ...
              <th className="harvest-num">
                {t("harvest.downloads.col.requests")}
              </th>
              <th className="harvest-num">{t("harvest.col.ok")}</th>
              <th className="harvest-num">{t("harvest.col.error")}</th>
              <th className="harvest-num">
                {t("harvest.downloads.col.empty")}
              </th>
              <th className="harvest-num">
                {t("harvest.downloads.col.ignored")}
              </th>
              <th className="harvest-num">
                {t("harvest.downloads.col.bytes")}
              </th>
          ...
          <tbody>
            {(datasets || []).map((row) => (
              <DatasetRow
                key={`${row.erddap_url}|${row.dataset_id}`}
                row={row}
                t={t}
              />
            ))}
        ...
        <table className="harvest-table">
          <thead>
            <tr>
              ...
              <th className="harvest-num">{t("harvest.col.ok")}</th>
              <th className="harvest-num">{t("harvest.col.error")}</th>
              <th className="harvest-num">{t("harvest.downloads.col.size")}</th>
          ...
          <tbody>
            {(jobs || []).map((j) => (
              <tr key={j.pk}>
                ...
                <td>
                  <span
                    className={`harvest-status harvest-status-${jobBadgeClass(j.status)}`}
                  >
                    {j.status}
                  </span>
                </td>
                <td className="harvest-num-total">{j.n_datasets}</td>
                <td className="harvest-num">{j.n_ok}</td>
                <td
                  className={`harvest-num ${Number(j.n_failed) > 0 ? "harvest-error-text" : ""}`}
                >
                  {j.n_failed}
                </td>
                <td className="harvest-num">{fmtBytes(j.download_size)}</td>
                <td className="harvest-muted harvest-text-sm">
