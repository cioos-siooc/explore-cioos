...
export default function HarvestRun() {
  ...
  const summary = attempts.reduce(
    (acc, a) => {
      acc[displayStatus(a)] = (acc[displayStatus(a)] || 0) + 1;
      acc.total++;
      return acc;
    },
  ...
  const breadcrumbs = (
    <>
      <Link to="/harvest">{t("harvest.title")}</Link> / {t("harvest.run.title")}{" "}
      {runId.slice(0, 8)}…
    </>
  );
  ...
  if (error)
    return (
      <HarvestLayout breadcrumbs={breadcrumbs}>
        <div className="harvest-fetch-error">{error}</div>
      </HarvestLayout>
    );
  if (!run)
    return (
      <HarvestLayout breadcrumbs={breadcrumbs}>
        <div className="harvest-fetch-error">{t("harvest.run.notFound")}</div>
      </HarvestLayout>
    );
  ...
  return (
    <HarvestLayout breadcrumbs={breadcrumbs}>
      ...
      <p className="harvest-page-sub harvest-mono harvest-text-sm">
        {run.run_id}
      </p>

      <div className="harvest-run-meta">
        <div className="harvest-run-meta-item">
          <span className="harvest-run-meta-label">
            {t("harvest.col.started")}
          </span>
          <span className="harvest-run-meta-value">
            {fmtDt(run.started_at)}
          </span>
        </div>
        <div className="harvest-run-meta-item">
          <span className="harvest-run-meta-label">
            {t("harvest.col.finished")}
          </span>
          <span className="harvest-run-meta-value">
        ...
        <div className="harvest-run-meta-item">
          <span className="harvest-run-meta-label">
            {t("harvest.col.duration")}
          </span>
          <span className="harvest-run-meta-value">
            {fmtDurationS(run.duration_s)}
          </span>
        </div>
        {run.git_sha && (
          <div className="harvest-run-meta-item">
            <span className="harvest-run-meta-label">
              {t("harvest.col.gitSha")}
            </span>
            <span className="harvest-run-meta-value harvest-mono">
      ...
      <div className="harvest-summary" style={{ marginBottom: "1rem" }}>
        ...
        <span
          className="harvest-count-pill harvest-count-unchanged"
          title={t("harvest.reason.UNCHANGED")}
        >
          ↻ {summary.unchanged}
        </span>
        ...
        <span className="harvest-muted harvest-text-md harvest-self-center">
      ...
      <table className="harvest-table">
        ...
        <tbody>
              ...
              <tr key={`${a.erddap_url}-${a.dataset_id}-${i}`}>
                ...
                <td>
                  <Link
                    ...
                    className="harvest-link harvest-mono harvest-text-sm"
                  >
                    {a.dataset_id}
                  </Link>
                </td>
                ...
                <td>
                  {a.reason_code && a.reason_code !== "UNCHANGED" && (
                    <span className="harvest-text-sm" title={a.reason_code}>
                      {reasonLabel(t, a.reason_code)}
                    </span>
                  )}
                  {a.error_message && (
                    <details style={{ marginTop: "0.2rem" }}>
                      <summary className="harvest-clickable harvest-text-xs harvest-error-text">
                        {t("harvest.run.detail")}
                      </summary>
                      <div
                        className="harvest-text-sm harvest-wrap"
          ...
          {!attempts.length && (
            <tr>
              <td colSpan="5" className="harvest-empty-row">
