...
describe("Switch", () => {
  ...
  it("calls onChange when toggled", async () => {
    ...
    render(
      <Switch id="s2" label="Bathymetry" checked={false} onChange={onChange} />,
    );
  ...
  it("falls back to the title as the accessible name when there is no visible label", () => {
    render(
      <Switch
        id="s4"
        title="Hide trajectories"
        checked={false}
        onChange={() => {}}
      />,
    );
    expect(
      screen.getByRole("switch", { name: "Hide trajectories" }),
    ).toBeInTheDocument();
  });
