...
def validate_harvest_config(yaml_text: str) -> str:
    ...
    a stale config is worse than refusing to start, so every failure is fatal.
    """
    try:
        parsed = yaml.safe_load(yaml_text)
    except yaml.YAMLError as e:
        raise ValueError(f"HARVEST_CONFIG_B64 is not valid YAML ({e}); {CONFIG_ENV_HINT}") from e
    if not isinstance(parsed, dict):
        raise ValueError(
            f"HARVEST_CONFIG_B64 holds {type(parsed).__name__}, expected a YAML mapping — it "
