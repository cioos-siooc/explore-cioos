...
Installed with the harvester package:
...
```bash
python -m cde_harvester.loading --folder harvest --incremental
```
...
```
python -m cde_harvester.loading --help
...
**Python Code** ([`loader.py`](loader.py)):
...
```bash
python -m cde_harvester.loading --folder harvest  # Without --incremental
```
