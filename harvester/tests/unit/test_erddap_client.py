...
class TestERDDAPInit:
    ...
    def test_empty_response_gives_empty_dataframe(self):
        ...
        mock_session = MagicMock()
        mock_session.get.return_value = MockResponse(text=empty_csv, url=ERDDAP_URL + "/tabledap/allDatasets.csv")
        with patch(
...
class TestErddapCsvToDf:
    def test_200_response_returns_dataframe(self):
        erddap = _make_erddap()
        erddap.session.get.return_value = MockResponse(text=ERDDAP_INFO_CSV, url=ERDDAP_URL + "/info/ds/index.csv")
    ...
    def test_404_returns_empty_dataframe(self):
        ...
        erddap.session.get.return_value = MockResponse(
            text="Not found", status_code=404, url=ERDDAP_URL + "/tabledap/missing.csv"
        )
    ...
    def test_500_no_matching_results_returns_empty(self):
        erddap = _make_erddap()
        erddap.session.get.return_value = MockResponse(
            text="Your query produced no matching results", status_code=500, url=ERDDAP_URL + "/tabledap/ds.csv"
        )
    ...
    def test_500_other_error_raises(self):
        erddap = _make_erddap()
        erddap.session.get.return_value = MockResponse(
            text="Internal Server Error", status_code=500, url=ERDDAP_URL + "/tabledap/ds.csv"
        )
    ...
    def test_response_too_large_raises(self):
        ...
        mock_resp = MockStreamingResponse(total_bytes=int(MAX_RESPONSE_SIZE) * 4, url=ERDDAP_URL)
    ...
    def test_streamed_body_decodes_like_response_text(self):
        ...
        erddap.session.get.return_value = MockResponse(text=csv, url=ERDDAP_URL + "/info/ds/index.csv")
