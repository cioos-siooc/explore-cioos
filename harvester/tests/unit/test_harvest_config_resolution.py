...
class TestValidateHarvestConfig:
    """A decoded env value funnels through here, so a mangled one never reaches a flow."""

    def test_returns_the_text_unchanged_for_a_mapping(self):
        assert validate_harvest_config(SAMPLE_YAML) == SAMPLE_YAML

    def test_rejects_a_non_mapping(self):
        with pytest.raises(ValueError, match="expected a YAML mapping"):
            validate_harvest_config("just a bare string")

    def test_rejects_unparseable_yaml(self):
        with pytest.raises(ValueError, match="not valid YAML"):
            validate_harvest_config("erddap_urls: [unclosed")
