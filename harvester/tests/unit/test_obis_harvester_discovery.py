...
DISCOVERY = ObisDiscoveryConfig.from_config({"enabled": True, "nodes": [NODE_A], "geometry": "none", "min_datasets": 0})
...
def stub_harvester(monkeypatch):
    ...
    class FakeHarvester:
        def __init__(self, limit_dataset_ids, folder, prefect_logger=None, geo_filter=None, run_id=None):
...
class TestHarvestObisDiscovery:
    def test_discovered_ids_are_passed_to_the_harvester(self, monkeypatch, stub_harvester, no_artifact):
    ...
    def test_explicit_ids_bypass_discovery_entirely(self, monkeypatch, stub_harvester, no_artifact):
    ...
    def test_disabled_discovery_is_not_constructed(self, monkeypatch, stub_harvester, no_artifact):
        seen = fake_discovery_class(monkeypatch, result=DiscoveryResult(dataset_ids=["z"]))
    ...
    def test_geo_filter_is_shared_with_discovery(self, monkeypatch, stub_harvester, no_artifact):
    ...
    def test_default_geo_filter_built_when_omitted(self, monkeypatch, stub_harvester, no_artifact):
