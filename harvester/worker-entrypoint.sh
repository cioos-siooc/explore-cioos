...
#
# --with-healthcheck serves /health on :8080 (PREFECT_WORKER_WEBSERVER_PORT);
# it goes unhealthy when the worker stops polling. The compose healthcheck
# probes it.
...
exec uv run prefect worker start --pool "${POOL_NAME}" --type process \
  --limit "${WORKER_LIMIT}" --with-healthcheck
