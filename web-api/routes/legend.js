...
router.get(
  ...
  async (req, res, next) => {
    ...
    const sql = `
        ...
        -- The metric aggregate, not count(distinct point_pk): the ramp ranks
        -- hexes by how much data they hold, so its domain has to be over the
        -- same quantity the tiles emit as \`count\`.
    ...
    const coverageInner = coverageBranches.length
      ...
      : `SELECT hex_1_pk, dataset_pk, ${nullMetricExpr(metric)} FROM cde.obis_cells WHERE FALSE`;
