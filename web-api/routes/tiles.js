 ...
 *           `days` counts the distinct UTC days the hex holds data for
 *           (overlapping deployments counted once); `datasets` counts distinct
...
router.get(
  ...
  async (req, res) => {
    ...
    WHERE show_as_point${profilesTypeFilter}${cellPrefilter ? ` AND ${cellPrefilter}` : ''}`;
    ...
    // used to be count(distinct point_pk) at hex zoom and a record sum at point
    // zoom, so the property changed meaning mid-zoom and the hex ramp ranked a
    // 20-year mooring level with a single CTD cast. Aggregating the same metric
    // at both tiers fixes the ramp and drops a distinct-aggregate at the same
    // time.
 ...
 *           ramp represents. `records` sums measurement/occurrence/fix counts;
 *           `days` counts the distinct UTC days the hex holds data for
 *           (overlapping deployments counted once). Anything else falls back to
