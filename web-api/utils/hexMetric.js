 ...
 * longer had a calendar meaning. One live hex read 451,305 days over a calendar
 * span of 5,197, and 59 republished copies of a single 375-day mooring
 * deployment made one marker read 21,996 days.
 *
 * Unlike the other two metrics it therefore can't be a per-row number summed
 * up: each row contributes a SET of days (`metricValueExpr` + `metricJoin`
 * below), and the aggregate unions them (`countAggregate` → day_union_days).
 *
...
//
// One expression serves all three tables because all three now carry the same
// column. `day_ranges` is the row's real day set — maximal runs of consecutive
// UTC days — so a station sampled every January contributes those days and not
// the decades between them. A NULL or empty array means "day set unknown", and
// the row falls back to its [time_min, time_max] span: that is what a row whose
// dataset has not been re-harvested since database/3_day_ranges_migration.sql
// contributes, and what cde.profiles rows of single-cast (Profile) datasets
// contribute permanently, where the span IS the day set.
//
// Upper bound exclusive, so a single-instant feature is one day. The GREATEST
// guards a time_max < time_min row, which daterange() would reject outright
// rather than tolerate. LEFT JOIN keeps a row with no usable days out of the
// union without dropping it from the hex's dataset list.
const DAY_SET_JOIN = `LEFT JOIN LATERAL (
      SELECT r AS day
        FROM unnest(day_ranges) r
       WHERE day_ranges IS NOT NULL
      UNION ALL
      SELECT daterange(time_min::date, GREATEST(time_max::date, time_min::date) + 1)
